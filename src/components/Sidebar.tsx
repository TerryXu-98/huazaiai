...
export default function Sidebar({ onAddNode, hiddenGroups = [], floating = false }: SidebarProps) {
  ...
  return (
    ...
    >
      ...
      >
        {isPixel ? (
          <span className="px-chip px-chip--muted">Huazai AI · v{__APP_VERSION__}</span>
        ) : (
          <>Huazai AI · v{__APP_VERSION__}</>
        )}
      </div>
    </div>
  );
