...
const MAGIC = Buffer.from('LIANG1\n', 'utf8'); // 7 bytes
const PASSPHRASE = 'Liang-2026';
...
function decryptBuffer(enc) {
  if (!isEncrypted(enc)) {
    throw new Error('[LIANG1] missing magic header');
  }
...
function generateScript(cachedData, filename) {
  ...
  if (script.cachedDataRejected) {
    throw new Error('[LIANG1] cachedDataRejected (V8 版本不匹配?请重新 npm run encrypt)');
  }
  return script;
}
